...
from ..parameters import FILE_PATH, COUNTRY_CELL, FILTER_CELL, COUNTRIES, FILTERS, AE_INDICATOR_MAPPING, JSON_FILE_AFOLU_EMISSIONS
...
def apply_changes_to_excel(prepared_data):
    ...
    for data in tqdm(prepared_data, desc="Processing", unit="combination"):
        try:
            ...
            target_column, target_row = AE_INDICATOR_MAPPING.get(indicator, ("F", 3))
...
if __name__ == "__main__":
    combinations = [(country, filter_, indicator) for country in COUNTRIES for filter_ in FILTERS for indicator in AE_INDICATOR_MAPPING.keys()]
    
    prepared_data = [prepare_data(combination) for combination in combinations]

    all_results = apply_changes_to_excel(prepared_data)

    with open(JSON_FILE_AFOLU_EMISSIONS, 'w') as json_file:
        json.dump(all_results, json_file, indent=2)

    print(f"Results have been written to {JSON_FILE_AFOLU_EMISSIONS}")