...
from ..parameters import FILE_PATH_POTENTIAL, COUNTRY_CELL, FILTER_CELL, COUNTRIES_POTENTIAL, FILTERS_POTENTIAL, P_INDICATOR_MAPPING, JSON_FILE_POTENTIAL
...
def apply_changes_to_excel(prepared_data):
    app = xw.App(visible=False)
    workbook = app.books.open(FILE_PATH_POTENTIAL)
    sheet = workbook.sheets[0]
    ...
    for data in tqdm(prepared_data, desc="Processing", unit="combination"):
        try:
            ...
            target_column, target_row = P_INDICATOR_MAPPING.get(indicator, ("F", 3))
...
if __name__ == "__main__":
    combinations = [(country, filter_, indicator) for country in COUNTRIES_POTENTIAL for filter_ in FILTERS_POTENTIAL for indicator in P_INDICATOR_MAPPING.keys()]
    
    prepared_data = [prepare_data(combination) for combination in combinations]

    all_results = apply_changes_to_excel(prepared_data)

    with open(JSON_FILE_POTENTIAL, 'w') as json_file:
        json.dump(all_results, json_file, indent=2)

    print(f"Results have been written to {JSON_FILE_POTENTIAL}")