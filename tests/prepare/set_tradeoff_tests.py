...
from ..parameters import FILE_PATH_TRADEOFF, COUNTRY_CELL, FILTER_CELL, COUNTRIES_TRADEOFF, FILTERS_TRADEOFF, T_INDICATOR_MAPPING, JSON_FILE_TRADEOFF
...
def apply_changes_to_excel(prepared_data):
    app = xw.App(visible=False)
    workbook = app.books.open(FILE_PATH_TRADEOFF)
    sheet = workbook.sheets[0]
    ...
    for data in tqdm(prepared_data, desc="Processing", unit="combination"):
        try:
            ...
            target_column, target_row = T_INDICATOR_MAPPING.get(indicator, ("F", 3))
...
if __name__ == "__main__":
    combinations = [(country, filter_, indicator) for country in COUNTRIES_TRADEOFF for filter_ in FILTERS_TRADEOFF for indicator in T_INDICATOR_MAPPING.keys()]
    
    prepared_data = [prepare_data(combination) for combination in combinations]

    all_results = apply_changes_to_excel(prepared_data)

    with open(JSON_FILE_TRADEOFF, 'w') as json_file:
        json.dump(all_results, json_file, indent=2)

    print(f"Results have been written to {JSON_FILE_TRADEOFF}")