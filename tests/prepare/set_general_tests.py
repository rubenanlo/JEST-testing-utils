...
from ..parameters import FILE_PATH, COUNTRY_CELL, FILTER_CELL, COUNTRIES, FILTERS, G_INDICATOR_MAPPING, JSON_FILE_GENERAL
...
# This function is used for any instances where the output array is an ordinal number string (e.g., "1st", "2nd", "3rd", "4th", etc.). We generate numbers instead with no decimals
def strip_ordinal_suffix(num_str):
    if len(num_str) > 2 and num_str[-2:].lower() in ['th', 'st', 'nd', 'rd']:
        return num_str[:-2]
    return num_str

def format_number(value, indicator, decimals=1):
    ...
    if isinstance(value, str) and ',' in value:
        ...
        for index, num_str in enumerate(number_strings):
            ...
            elif num_str[:-2].isdigit() and num_str[-2:].lower() in ['th', 'st', 'nd', 'rd']:
                formatted_numbers.append(int(strip_ordinal_suffix(num_str)))
            else:
...
def apply_changes_to_excel(prepared_data):
    ...
    for data in tqdm(prepared_data, desc="Processing", unit="combination"):
        try:
            ...
            target_column, target_row = G_INDICATOR_MAPPING.get(indicator, ("F", 3))
            target_cell_address = f"{target_column}{target_row}"
            
            cell_value = sheet.range(target_cell_address).value
            formatted_values = format_number(cell_value, indicator, decimals=1)

            # Check if formatted_values is None
            if formatted_values is None:
                continue  # Skip adding this data to results
...
if __name__ == "__main__":
    combinations = [(country, filter_, indicator) for country in COUNTRIES for filter_ in FILTERS for indicator in G_INDICATOR_MAPPING.keys()]
    
    prepared_data = [prepare_data(combination) for combination in combinations]

    all_results = apply_changes_to_excel(prepared_data)

    with open(JSON_FILE_GENERAL, 'w') as json_file:
        json.dump(all_results, json_file, indent=2)

    print(f"Results have been written to {JSON_FILE_GENERAL}")